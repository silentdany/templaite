...
async function rmRel(targetDir: string, rel: string) {
  const p = path.join(targetDir, rel);
  if (existsSync(p)) await rm(p, { recursive: true, force: true });
}

async function copyAsset(name: string, targetDir: string, destRel: string) {
...
async function pruneNoAuth(targetDir: string, features: TemplaiteFeatures) {
  const remove = [
    "prisma",
    "prisma.config.ts",
    "src/lib/prisma.ts",
  ...
  for (const rel of remove) await rmRel(targetDir, rel);
  ...
  await copyAsset(
    features.notionBlog
      ...
      : "marketing-footer-no-auth.tsx",
    targetDir,
    "src/components/marketing/sections/marketing-footer.tsx",
  );

  await copyAsset(
    "pricing-no-payments.tsx",
    targetDir,
  ...
  if (!features.notionBlog) {
    await writeFile(
      path.join(targetDir, "src/app/sitemap.ts"),
      buildSitemapTs(),
      "utf8",
    );
  }
  ...
  await writeFile(
    pkgPath,
    `${JSON.stringify(filterPackageJson(pkg, features), null, 2)}\n`,
    "utf8",
  );
...
async function patchMarketingStringsNoAuth(targetDir: string) {
...
async function pruneWithAuth(targetDir: string, features: TemplaiteFeatures) {
  ...
  await writeFile(
    ...
    buildAuthTs(features),
    "utf8",
  );

  if (!features.googleOAuth) {
    await writeFile(
      path.join(targetDir, "src/lib/google-auth.ts"),
      `/** Google OAuth disabled at scaffold time. */
...
`,
  ...
  await writeFile(
    ...
    buildProxyTs(features),
    "utf8",
  );

  await writeFile(
    path.join(targetDir, "src/app/(app)/layout.tsx"),
    buildAppLayoutTs(features),
    "utf8",
  );

  await writeFile(
    path.join(targetDir, "src/app/(app)/playground/page.tsx"),
    buildPlaygroundPageTs(features),
    "utf8",
  );
  ...
  if (!features.notionBlog) {
    await copyAsset(
      "marketing-footer-auth-no-blog.tsx",
      targetDir,
      "src/components/marketing/sections/marketing-footer.tsx",
    );
...
async function patchFooterAuthOptionalLinks(
  targetDir: string,
  features: TemplaiteFeatures,
) {
  const p = path.join(
...
async function patchMarketingHeaderRemoveBlog(targetDir: string) {
  const p = path.join(
    targetDir,
    "src/components/marketing/marketing-header.tsx",
  );
...
async function patchRootLayoutKeywords(
  targetDir: string,
  features: TemplaiteFeatures,
) {
  const p = path.join(targetDir, "src/app/layout.tsx");
  let s = await readFile(p, "utf8");
  const keywords: string[] = ["Next.js", "shadcn", "boilerplate"];
  if (features.auth) {
...
async function writeManifestAndEnv(
  targetDir: string,
  features: TemplaiteFeatures,
) {
  await writeFile(
    path.join(targetDir, ".templaite.json"),

...
import { DEFAULT_FEATURES, featuresFromPrompts, validateFeatures, } from "./features.js";
import { pruneProject } from "./prune.js";
const __dirname = path.dirname(fileURLToPath(import.meta.url));
function resolveDefaultTemplateRoot() {
...
async function run() {
    ...
    let projectName = args.targetName ??
        (await p.text({
...
async function runNonInteractive(args) {
    ...
    if (!projectName?.trim()) {
        console.error("Usage: create-templaite --defaults <dir> [--minimal] [--from <template>] [--skip-install]");
        process.exit(1);
    }
